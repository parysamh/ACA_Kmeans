...
int main() {
    ...
    // --- timing start (clustering only) ---
    auto t0 = std::chrono::high_resolution_clock::now();

    // Run K-Means
    double tol = 1e-4; // tolerance for centroid movement
    int converged_after = 100; // default to max if it never breaks earlier
    for (int iter = 0; iter < 100; ++iter) { // max 100 iterations
        ...
        for (int c = 0; c < K; ++c) {
            double dx = centroids[c][0] - prev_centroids[c][0];
            double dy = centroids[c][1] - prev_centroids[c][1];
            double shift = std::sqrt(dx*dx + dy*dy);
            if (shift > max_shift) max_shift = shift;
        }

        if (max_shift < tol) {
            converged_after = iter + 1;
            cout << "Converged after " << converged_after << " iterations\n";
            break;
        }
    }
